...
Where the Gerrit instance grants anonymous read, set `gerrit.anonymous` and
omit the token. The client then uses the unauthenticated endpoints instead of
the `/a/` prefix. The archiver only ever issues GETs, so this costs nothing
except that anonymous access cannot see private changes at all -- a safe
...
Separately run against a live Gerrit 3.3.0 instance: the sweep query,
all six `o=` detail options and the `-is:private` and `after:` operators are
accepted, and a dry-run sweep completes cleanly.

Note that `context_lines` requires the `enable-context` request parameter,
spelled with a hyphen. Some versions' documentation writes it with an
underscore, which the server rejects with 400; the client sends the hyphen
form and retries without it if a server objects.
