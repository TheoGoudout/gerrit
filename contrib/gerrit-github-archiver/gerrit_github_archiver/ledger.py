...
-- One row per artefact projected onto GitHub. The primary key is the
...
class Ledger:
    ...
    The webhook workers and the periodic sweep run concurrently, so the
    connection is shared across threads and every statement runs under one
    lock; SQLite would otherwise refuse cross-thread use outright. Volume is
    low enough that a single lock costs nothing.
    """
    ...
    def record_patchset(self, key: str, number: int, sha: str) -> None:
        with self._lock:
            self._conn.execute(
                "INSERT OR REPLACE INTO patchsets (change_key, number, sha) "
                "VALUES (?, ?, ?)",
                (key, number, sha),
            )
    ...
    def is_synced(self, key: str, kind: str, gerrit_id: str) -> bool:
        with self._lock:
            row = self._conn.execute(
                ...
                "WHERE change_key = ? AND kind = ? AND gerrit_id = ?",
                (key, kind, gerrit_id),
            ).fetchone()
        ...
        with self._lock:
            cur = self._conn.execute(
                ...
                INSERT OR IGNORE INTO synced_items
                    (change_key, kind, gerrit_id, github_id)
                VALUES (?, ?, ?, ?)
                """,
                (
                    key,
                    kind,
                    gerrit_id,
                    str(github_id) if github_id is not None else None,
                ),
            )
